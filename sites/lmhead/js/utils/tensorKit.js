...
import { THEME, EMISSIVE, isLight } from './sceneKit.js';

const PALE = new THREE.Color( THEME.bg );

export function cellMesh( cell, depth, color = THEME.off ) {

	return new THREE.Mesh(
		new THREE.BoxGeometry( cell, cell, depth ),
		new THREE.MeshStandardMaterial( { color, emissive: color, emissiveIntensity: 0.2 * EMISSIVE, roughness: 0.45, metalness: 0.2 } ),
	);
...
export function paintCell( mesh, color, level = 0.5 ) {
	...
	// A low level has to read as *less* than the stage, so on a dark stage it
	// darkens toward black and on a light one it fades toward the background.
	// Neither end may vanish: a dim cell is still information.
	if ( isLight ) mesh.material.color.lerp( PALE, 0.5 - level * 0.48 );
	else mesh.material.color.multiplyScalar( 0.32 + level * 0.85 );

	mesh.material.emissiveIntensity = ( 0.05 + level * 0.85 ) * EMISSIVE;
...
export function createBar( { width = 0.6, depth = 0.6, color = THEME.accent } = {} ) {
	...
	const mat = new THREE.MeshStandardMaterial( { color, emissive: color, emissiveIntensity: 0.45 * EMISSIVE, roughness: 0.4, metalness: 0.2 } );
...
export function createRail( from, to, { color = THEME.muted, radius = 0.035, opacity = 0.7 } = {} ) {
	...
	const mesh = new THREE.Mesh(
		...
		new THREE.MeshStandardMaterial( { color, emissive: color, emissiveIntensity: 0.35 * EMISSIVE, transparent: true, opacity } ),
	);
